...
namespace RESTful_API.Repositories
{
    ...
    public class UserAccountRepository
    {
        ...
        public async Task CreateUserAccount(UserAccount userAccount)
        {
            ...
            try
            {
                const string insertQuery = @"INSERT INTO [UserAccount] (Email, Password, CustomerID)
                                    VALUES (@Email, @Password, @CustomerID)";

                await dbConnection.ExecuteAsync(insertQuery, userAccount, transaction: transaction);
                transaction.Commit();
